/// LABEL for All-but-One OT
pub const ALL_BUT_ONE_LABEL: &[u8] =
    b"SilenceLaboratories-ALL-BUT-ONE-OT-v1.0";

/// LABEL for All-but-One PPRF
pub const ALL_BUT_ONE_PPRF_LABEL: &[u8] = b"ALL-BUT-ONE-OT-PPRF";

/// LABEL for All-but-One PPRF Hash
pub const ALL_BUT_ONE_PPRF_HASH_LABEL: &[u8] = b"ALL-BUT-ONE-OT-PPRF-HASH";

/// LABEL for All-but-One PPRF Proof
pub const ALL_BUT_ONE_PPRF_PROOF_LABEL: &[u8] = b"ALL-BUT-ONE-OT-PPRF-PROOF";

/// LABEL for SoftSpokenOT
pub const SOFT_SPOKEN_LABEL: &[u8] =
    b"SilenceLaboratories-SOFT-SPOKEN-OT-v1.0";

/// LABEL for SoftSpokenOT expand seeds
pub const SOFT_SPOKEN_EXPAND_LABEL: &[u8] = b"SOFT-SPOKEN-OT-EXPAND";

/// LABEL for SoftSpokenOT matrix hash
pub const SOFT_SPOKEN_MATRIX_HASH_LABEL: &[u8] =
    b"SOFT-SPOKEN-OT-MATRIX-HASH";

/// LABEL for SoftSpokenOT randomize
pub const SOFT_SPOKEN_RANDOMIZE_LABEL: &[u8] = b"SOFT-SPOKEN-OT-RANDOMIZE";
